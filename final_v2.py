...
REQUEST_COUNT = 0
LAST_REQUEST_TIME = 0
MIN_REQUEST_INTERVAL = 1.2  # 60 seconds / 50 requests = 1.2 seconds per request
...
def generate_queries():
    ...
    return [ char1 + char2 for char1 in CHARS for char2 in CHARS]
...
def main():
    ...
    try:
        for i, query in enumerate(queries, 1):
            ...
            if i % 50 == 0:
                with open("partial_results_v2.json", "w") as f:
                    json.dump({
    ...
    with open("final_results_v2.json", "w") as f:
        json.dump({
