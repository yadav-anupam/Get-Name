...
REQUEST_COUNT = 0
LAST_REQUEST_TIME = 0
MIN_REQUEST_INTERVAL = 0.75  # 60 seconds / 80 requests =  0.75seconds per request

FIRST_CHARS = string.digits + string.ascii_lowercase  # 0-9, a-z
NEXT_CHARS = "-."  + FIRST_CHARS  # Allowed subsequent characters
...
def generate_queries():
    return [ char1 for char1 in FIRST_CHARS]
...
def fetch_recursive(query):
    ...
    try:
        ...
        return [item for c in NEXT_CHARS
...
def main():
    ...
    try:
        for i, query in enumerate(queries, 1):
            ...
            if i % 50 == 0:
                with open("partial_results_v3.json", "w") as f:
                    json.dump({
    ...
    with open("final_results_v3.json", "w") as f:
        json.dump({
